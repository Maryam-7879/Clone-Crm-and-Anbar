...
$page_title = $is_edit ? 'ویرایش تراکنش' : 'تراکنش جدید';
        ...
        if (empty($errors)) {
            try {
                ...
                if ($is_edit) {
                    ...
                    $reference_number = $reference_number ?: 'TXN-' . time();
                    ...
                    ");
                    $stmt->execute([
                        $product_id, $_SESSION['user_id'], $type, $quantity, $unit_price,
                        $total_price, $notes, $reference_number, $transaction_date
                    ]);
                ...
                logActivity($_SESSION['user_id'], $action . '_transaction', 'transactions', $transaction_id);
                ...
                error_log("خطا در ذخیره تراکنش: " . $e->getMessage());
                    ...
                    <div class="row">
                            ...
                            <select class="form-select" name="product_id" id="product_id" required>
                                ...
                                <?php foreach ($products as $prod): ?>
                                    <option value="<?php echo $prod['id']; ?>" 
                                            <?php echo (isset($transaction) && $transaction['product_id'] == $prod['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" name="type" id="type" required>
                                ...
                                <option value="in" <?php echo (isset($transaction) && $transaction['type'] === 'in') ? 'selected' : ''; ?>>ورود به انبار</option>
                                <option value="out" <?php echo (isset($transaction) && $transaction['type'] === 'out') ? 'selected' : ''; ?>>خروج از انبار</option>
                                <option value="adjustment" <?php echo (isset($transaction) && $transaction['type'] === 'adjustment') ? 'selected' : ''; ?>>تعدیل موجودی</option>
                                <option value="transfer" <?php echo (isset($transaction) && $transaction['type'] === 'transfer') ? 'selected' : ''; ?>>انتقال</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="datetime-local" class="form-control" name="transaction_date" 
                                   value="<?php echo isset($transaction['transaction_date']) ? date('Y-m-d\TH:i', strtotime($transaction['transaction_date'])) : date('Y-m-d\TH:i'); ?>" 
                                   required>
                        </div>
